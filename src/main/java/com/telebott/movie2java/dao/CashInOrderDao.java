...
public interface CashInOrderDao extends JpaRepository<CashInOrder, Long>, CrudRepository<CashInOrder, Long> {
    ...
    @Query(value = "SELECT cio.* FROM `cash_in_order` AS cio INNER JOIN `diamond_order` go ON go.order_no = cio.order_no AND go.user_id =:userId WHERE cio.order_type=3",nativeQuery = true)
    Page<CashInOrder> getAllByDiamond(long userId, Pageable pageable);
    @Query(value = "SELECT cio.* FROM `cash_in_order` AS cio INNER JOIN `coin_order` go ON go.order_no = cio.order_no AND go.user_id =:userId WHERE cio.order_type=5",nativeQuery = true)
    Page<CashInOrder> getAllByCoin(long userId, Pageable pageable);
    @Query(value = "SELECT cio.* FROM `cash_in_order` AS cio INNER JOIN `cash_order` go ON go.order_no = cio.order_no AND go.user_id =:userId WHERE cio.order_type=2",nativeQuery = true)
    Page<CashInOrder> getAllByCash(long userId, Pageable pageable);
    @Query(value = "SELECT cio.* FROM `cash_in_order` AS cio INNER JOIN `membership_order` go ON go.order_no = cio.order_no AND go.user_id =:userId WHERE cio.order_type=1",nativeQuery = true)
    Page<CashInOrder> getAllByMembership(long userId, Pageable pageable);
