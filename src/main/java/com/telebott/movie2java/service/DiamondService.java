...
public class DiamondService {
    ...
    public ResponseData balance(User user, String ip) {
        ...
        return ResponseData.success(ResponseData.object("balance",userBalanceDiamondDao.getAllByBalance(user.getId())));
    }
    public ResponseData buttons(User user, String ip) {
        if (user == null) return ResponseData.error("");
        List<DiamondButton> buttons = diamondButtonDao.getAllButtons();
        JSONArray array = new JSONArray();
        for (DiamondButton b: buttons) {
...
//                    List<CashInOption> o = cashInOptionDao.findAllByStatusAndCode(1, s);
    ...
    public ResponseData payment(long id, long toId, String schema, String serverName, int serverPort, User user, String ip) {
        ...
        DiamondOrder order = new DiamondOrder();
        ...
        diamondOrderDao.saveAndFlush(order);
...
//        System.out.printf("%s\n",sb.toString());
    ...
    public ResponseData order(int page, User user, String ip) {
        ...
        Page<CashInOrder> orderPage = cashInOrderDao.getAllByDiamond(user.getId(),pageable);
        JSONArray array = new JSONArray();
        for (CashInOrder order : orderPage.getContent()){
            DiamondOrder diamondOrder = diamondOrderDao.findAllByOrderNo(order.getOrderNo());
...
//        System.out.printf("array%s\n", array);
    ...
    public ResponseData fund(int page, User user, String ip) {
        ...
        if (page < 0) page = 0;
        Pageable pageable = PageRequest.of(page,12,Sort.by(Sort.Direction.DESC,"id"));
        Page<UserBalanceDiamond> fundsPage = userBalanceDiamondDao.findAllByUserId(user.getId(), pageable);
        JSONArray array = new JSONArray();
        for (UserBalanceDiamond funds : fundsPage.getContent()){
            JSONObject json = new JSONObject();
